...
@section('content')
    ...
    <div class="container-fluid">
        <div class="content-header">
            <h2><i class="fas fa-edit me-2"></i> Modifier l'Impact</h2>
        </div>

        <div class="form-card">
            <form action="{{ route('content.advantage-impacts.update', $impact->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('value') is-invalid @enderror" id="value"
                        name="value" value="{{ old('value', $impact->value) }}" placeholder="Ex: 95%" required>
                    @error('value')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('label') is-invalid @enderror" id="label"
                        name="label" value="{{ old('label', $impact->label) }}" placeholder="Ex: de satisfaction"
                        required>
                    @error('label')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('order') is-invalid @enderror" id="order"
                        name="order" value="{{ old('order', $impact->order) }}" min="1" required>
                    @error('order')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1"
                            {{ old('is_active', $impact->is_active) ? 'checked' : '' }}>
