            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Modifier le témoignage</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('content.testimonials.update', $testimonial) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('display_on') is-invalid @enderror" id="display_on"
                                    ...
                                    <option value="home"
                                        {{ old('display_on', $testimonial->display_on) == 'home' ? 'selected' : '' }}>
                                        Accueil</option>
                                    <option value="avantages"
                                        {{ old('display_on', $testimonial->display_on) == 'avantages' ? 'selected' : '' }}>
                                        Avantages</option>
                                    <option value="both"
                                        {{ old('display_on', $testimonial->display_on) == 'both' ? 'selected' : '' }}>Les
                                        deux</option>
                                </select>
                                @error('display_on')
                                    <span class="invalid-feedback">{{ $message }}</span>
